...
contains
  ...
  function as_dmatrix_csc(sparse) result(matrix)
    class(sparse_dmatrix_csc),intent(in)            :: sparse
    real(dp),dimension(sparse%Nrow,sparse%Ncol) :: matrix
    integer                                    :: i,j
    matrix = 0d0
    do j=1,sparse%Ncol
       do i=1,sparse%col(j)%Size
  ...
  function as_zmatrix_csc(sparse) result(matrix)
    class(sparse_zmatrix_csc),intent(in)            :: sparse
    complex(dp),dimension(sparse%Nrow,sparse%Ncol) :: matrix
    integer                                    :: i,j
    matrix = 0d0
    do j=1,sparse%Ncol
       do i=1,sparse%col(j)%Size
  ...
  ! type[Const]=integer(4),real(dp),cmplx(dp)
  !+------------------------------------------------------------------+
  ...
  function left_matmul_zmatrix_zarray_csc(C,A) result(B)
    complex(dp),intent(in),dimension(:)  :: C
    type(sparse_zmatrix_csc),intent(in) :: A
    complex(dp),dimension(size(C))       :: B
    integer                             :: i,j
    B=0.d0
